...
import { Brain, Activity, BarChart2, Zap, AlertCircle, CheckCircle, Info, ChevronDown } from 'lucide-react'
...
const AdminMLAnalytics = () => {
  ...
  if (infoError) return (
    <div style={{ maxWidth: 700 }}>
      <h1 style={{ fontSize: 24, fontWeight: 800, marginBottom: 8 }}>ML Analytics</h1>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: '#b91c1c', lineHeight: 1.7 }}>
              The ML microservice is not running. Start it with:
            </p>
    ...
    <div style={{ maxWidth: 1100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>
              Random Forest classifier trained on 1,205 student records · {modelInfo ? `${modelInfo.accuracy}% accuracy` : 'Loading...'}
            </p>
          </div>
        </div>
      </div>
        ...
        <div>
          {loadingInfo ? (
            <div className="stat-grid" style={{ marginBottom: 24 }}>
              {[1,2,3,4].map(i => <Sk key={i} w="100%" h={100} />)}
            </div>
          ) : modelInfo && (
            <>
              <div className="stat-grid" style={{ marginBottom: 24 }}>
                    ...
                    return (
                      <div key={label} style={{ marginBottom: 14 }}>
                        ...
                        <div style={{ height: 8, background: 'var(--page-bg-2)', borderRadius: 4, overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${pct}%`, background: levelColor[label], borderRadius: 4, transition: 'width 0.8s ease' }} />
                        </div>
                      </div>
                    )
                  })}
              ...
              <div style={card}>
                ...
                <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>
                  Rows = Actual, Columns = Predicted (High / Low / Moderate)
                </p>
                    ...
                    <tbody>
                      {modelInfo.confusion_matrix.map((row, i) => (
                        <tr key={i} style={{ borderTop: '1px solid var(--card-border)' }}>
                          <td style={{ padding: '10px 16px', fontWeight: 700, color: levelColor[['High','Low','Moderate'][i]] }}>
                            {['High','Low','Moderate'][i]}
                          </td>
                          {row.map((val, j) => (
                            <td key={j} style={{
                              padding: '10px 20px', textAlign: 'center', fontWeight: i === j ? 800 : 400,
                              background: i === j ? `${levelColor[['High','Low','Moderate'][i]]}18` : 'transparent',
                              color: i === j ? levelColor[['High','Low','Moderate'][i]] : 'var(--text-secondary)'
                            }}>
                              {val}
                            </td>
                          ))}
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.7 }}>
                      ...
                      from the Student Adaptability in Online Education dataset. The model uses balanced class weights
                      to handle the dataset's class imbalance (High: 8%, Low: 40%, Moderate: 52%).
                      Feature importance is calculated using mean impurity decrease across all trees.
                      5-fold cross-validation confirms consistent performance of <strong>{modelInfo.cv_accuracy}% ± {modelInfo.cv_std}%</strong>.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div style={card}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <Btn
                ...
                onClick={() => predictMutation.mutate(form)}
                icon={<Zap size={15} />}
                style={{ marginTop: 8 }}
              >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ ...card, background: levelBg[prediction.prediction], border: `2px solid ${levelColor[prediction.prediction]}30` }}>
                  <div style={{ textAlign: 'center', padding: '12px 0' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: levelColor[prediction.prediction], textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>
                      Predicted Adaptability Level
                    </div>
                    <div style={{ fontSize: 48, fontWeight: 900, color: levelColor[prediction.prediction], letterSpacing: '-0.04em', marginBottom: 8 }}>
                ...
                <div style={card}>
                  ...
                  {Object.entries(prediction.probabilities).sort((a,b) => b[1]-a[1]).map(([label, pct]) => (
                    <div key={label} style={{ marginBottom: 12 }}>
                      ...
                      <div style={{ height: 8, background: 'var(--page-bg-2)', borderRadius: 4, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${pct}%`, background: levelColor[label], borderRadius: 4, transition: 'width 0.6s ease' }} />
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {activeTab === 'insights' && (
        <div>
          {loadingInsights ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[1,2,3].map(i => <Sk key={i} w="100%" h={200} />)}
            </div>
          ) : insights && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={card}>
                <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>Adaptability by Financial Condition</h3>
                <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>
                  Financial condition is the strongest predictor (17.1% importance)
                </p>
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  {Object.entries(insights.by_financial_condition).map(([cond, levels]) => {
                    const total = Object.values(levels).reduce((a,b) => a+b, 0)
                    return (
                      <div key={cond} style={{ flex: 1, minWidth: 150, padding: '14px 16px', background: 'var(--page-bg-2)', borderRadius: 10, border: '1px solid var(--card-border)' }}>
                        <div style={{ fontWeight: 700, marginBottom: 12, fontSize: 14 }}>{cond}</div>
                        {Object.entries(levels).map(([level, count]) => (
                          <div key={level} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                            ...
                            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{count} ({((count/total)*100).toFixed(0)}%)</span>
                          </div>
                        ))}
                      </div>
                    )
              ...
              {/* By Internet Type */}
              <div style={card}>
                <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>Adaptability by Internet Type</h3>
                <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>
                  Particularly relevant for Nigerian university students relying on mobile data
                </p>
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  {Object.entries(insights.by_internet_type).map(([type, levels]) => {
                    const total = Object.values(levels).reduce((a,b) => a+b, 0)
                    return (
                      <div key={type} style={{ flex: 1, minWidth: 200, padding: '14px 16px', background: 'var(--page-bg-2)', borderRadius: 10, border: '1px solid var(--card-border)' }}>
                        <div style={{ fontWeight: 700, marginBottom: 12, fontSize: 14 }}>{type}</div>
                        {Object.entries(levels).map(([level, count]) => (
                          <div key={level} style={{ marginBottom: 10 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                              <span style={{ fontSize: 12, fontWeight: 600, color: levelColor[level] }}>{level}</span>
                              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{((count/total)*100).toFixed(1)}%</span>
                            </div>
                            <div style={{ height: 6, background: 'var(--card-border)', borderRadius: 3 }}>
              ...
              {/* By Device */}
              <div style={card}>
                <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16 }}>Adaptability by Device Type</h3>
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  {Object.entries(insights.by_device).map(([device, levels]) => {
                    const total = Object.values(levels).reduce((a,b) => a+b, 0)
                    return (
                      <div key={device} style={{ flex: 1, minWidth: 150, padding: '14px 16px', background: 'var(--page-bg-2)', borderRadius: 10, border: '1px solid var(--card-border)' }}>
                        <div style={{ fontWeight: 700, marginBottom: 12, fontSize: 14 }}>{device}</div>
                        {Object.entries(levels).map(([level, count]) => (
              ...
              {/* By Network Type */}
              <div style={card}>
                <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16 }}>Adaptability by Network Type</h3>
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  {Object.entries(insights.by_network_type).map(([net, levels]) => {
                    const total = Object.values(levels).reduce((a,b) => a+b, 0)
                    return (
                      <div key={net} style={{ flex: 1, minWidth: 120, padding: '14px 16px', background: 'var(--page-bg-2)', borderRadius: 10, border: '1px solid var(--card-border)' }}>
                        <div style={{ fontWeight: 700, marginBottom: 12, fontSize: 14 }}>{net}</div>
                        {Object.entries(levels).map(([level, count]) => (
                          <div key={level} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                            <Badge color={levelBadge[level]}>{level}</Badge>
                            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{count} ({((count/total)*100).toFixed(0)}%)</span>
              ...
              <div style={{ ...card, background: 'var(--blue-50)', border: '1px solid var(--blue-200)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[
                    ...
                    'Students using WiFi consistently show higher adaptability rates compared to mobile data users — relevant to Nigerian campuses with poor connectivity.',
                    ...
                    <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 14px', background: 'rgba(255,255,255,0.7)', borderRadius: 8 }}>
                      <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--blue-600)', flexShrink: 0, marginTop: 1 }}>{i+1}.</span>
