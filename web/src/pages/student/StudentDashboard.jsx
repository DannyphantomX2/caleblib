...
import { BookOpen, Download, Bookmark, Bell, Search, ArrowRight, FileText, Clock } from 'lucide-react'
    ...
    <div style={{ maxWidth: 1100 }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>
          {user?.academicLevel} Level · Computer Science · {new Date().toLocaleDateString('en-NG', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 28 }}>
        {isLoading ? [1,2,3].map(i => <SkStat key={i} />) : (
          <>
            <StatCard label="Total Resources" value={data?.stats?.totalResources ?? 0} icon={BookOpen} color="var(--blue-600)" sub="Available in library" />
            <StatCard label="My Bookmarks" value={data?.stats?.bookmarkCount ?? 0} icon={Bookmark} color="#8b5cf6" sub="Saved resources" />
            <StatCard label="Announcements" value={data?.announcements?.length ?? 0} icon={Bell} color="#f59e0b" sub="Active notices" />
          </>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24 }}>
        ...
        <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 'var(--radius)', boxShadow: 'var(--card-shadow)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 20px', borderBottom: '1px solid var(--card-border)' }}>
            ...
            <button onClick={() => navigate('/student/library')} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'var(--blue-600)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, fontFamily: 'var(--font)' }}>
              Browse all <ArrowRight size={14} />
            </button>
          </div>
          <div style={{ padding: '8px 8px' }}>
            {isLoading ? [1,2,3,4,5].map(i => <SkRow key={i} />) : data?.recentResources?.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {data?.announcements?.length > 0 && (
            <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 'var(--radius)', boxShadow: 'var(--card-shadow)', padding: '18px 20px' }}>
              <h2 style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>Announcements</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {data.announcements.map(a => (
                  <div key={a._id} style={{ padding: '10px 12px', background: a.isPinned ? '#fffbeb' : 'var(--page-bg-2)', borderRadius: 8, borderLeft: `3px solid ${a.isPinned ? '#f59e0b' : 'var(--blue-500)'}` }}>
                    <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>{a.title}</div>
                    <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>{truncate(a.content, 80)}</div>
