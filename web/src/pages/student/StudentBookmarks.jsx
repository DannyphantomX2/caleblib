...
const StudentBookmarks = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['bookmarks'],
    queryFn: async () => { const r = await api.get('/student/bookmarks'); return r.data }
  })

  const removeMutation = useMutation({
    mutationFn: async (id) => { const r = await api.post(`/student/bookmarks/${id}`); return r.data },
    onSuccess: () => { toast.success('Bookmark removed'); queryClient.invalidateQueries(['bookmarks']) }
  })
    ...
    <div style={{ maxWidth: 900 }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>
          {data?.bookmarks?.length ? `${data.bookmarks.length} saved resource${data.bookmarks.length !== 1 ? 's' : ''}` : 'Resources you save will appear here'}
        </p>
      </div>
      ...
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 'var(--radius)', boxShadow: 'var(--card-shadow)' }}>
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Browse the library and bookmark resources to find them quickly later</p>
          ...
          <div>
            {data.bookmarks.map((b, i) => {
              ...
              if (!r) return null
              return (
                <div key={b._id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', borderBottom: i < data.bookmarks.length - 1 ? '1px solid var(--card-border)' : 'none', transition: 'var(--transition)' }}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={() => navigate(`/student/resources/${r._id}`)}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <Btn size="xs" variant="outline" icon={<Download size={12} />}
                      onClick={() => downloadMutation.mutate({ id: r._id, fileName: r.fileName })}
                      loading={downloadMutation.isPending}>
                      Download
                    </Btn>
                    <Btn size="xs" variant="ghost" icon={<BookmarkX size={13} />}
